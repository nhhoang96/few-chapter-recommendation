...
WORD = re.compile(r'\w+')
csv_output = open('Summary_More_Chapter_' + argv[1] +'.csv', 'w')
...
###-----------------------Benchmark (few-chapter book only) -------------------------- #####
num_topics.append(len(book_topics))
related_topics =set() # No related topics in this case
conf_rank_matrix, rank_feature = calculate_rank_matrix(book_topics_list, book_topics_list, related_topics,book_topic_weights)
rank_conf_score = use_rank_matrix(full_conf_topics_list, rank_feature)
rank_extended_score = use_rank_matrix(book_topics_list, rank_feature)
ranked_similarity = rank_conf_score / rank_extended_score
...
print ("Original Few Chapter Book")
...
csv_writer.writerow(['Original', len(book_topics), ranked_similarity, jaccard_similarity, cosine_simlarity])


##-----Build extended chapters by combining few-chapter books with many-chapter books---------
#----------------------either separately or cumulatively---------------------------------------
full_extended_chapters = []
included_paper =[]
xlabels = []
num_topics = []

xlabels.append('O')
with open('long_book.json') as f:
    data = json.load(f, object_pairs_hook=collections.OrderedDict)
    for key, value in data.items():
        xlabels.append(key.split('_')[1])
        full_extended_chapters.append(value)

xlabels.append('All')

for index in range (len(full_extended_chapters)):
    if (argv[1] != 'cumulative'):
        included_paper = []

    included_paper.append(full_extended_chapters[index])
    extended_topics, extended_topics_list, related_topics = compute_topics(included_paper,clf,book_topics)
    num_topics.append(len(extended_topics))

    conf_rank_matrix, rank_feature = calculate_rank_matrix(book_topics_list, extended_topics_list, related_topics,book_topic_weights)
    rank_conf_score = use_rank_matrix(full_conf_topics_list, rank_feature)
    rank_extended_score = use_rank_matrix(extended_topics_list, rank_feature)
    ranked_similarity = rank_conf_score / rank_extended_score

    conf_vec = text_to_vector(' '.join(full_conf_topics_list))
    book_vec = text_to_vector(' '.join(extended_topics_list))
    cosine_simlarity = compute_cosine_similarity(conf_vec, book_vec)
    jaccard_similarity = compute_jaccard_similarity(conf_vec, book_vec)
    cosine_similarities.append(cosine_simlarity)
    jaccard_similarities.append(jaccard_similarity)
    ...
    print ("Chapter: ", xlabels[index + 1])
    print ("Number of topics: ", len(extended_topics))
    print ("Ranked", ranked_similarity)
    print ("Jaccard", jaccard_similarity)
    print ("Cosine", cosine_simlarity)
    print ("\n")

    csv_writer.writerow([xlabels[index + 1], len(extended_topics), ranked_similarity, jaccard_similarity, cosine_simlarity])

# ### Full Training (few-chapters and ALL chapters of many-chapters book) #####
extended_topics, extended_topics_list, related_topics = compute_topics(full_extended_chapters, clf, book_topics)
num_topics.append(len(extended_topics))
conf_rank_matrix, rank_feature = calculate_rank_matrix(book_topics_list, extended_topics_list, related_topics, book_topic_weights)
rank_conf_score = use_rank_matrix(full_conf_topics_list, rank_feature)
rank_extended_score = use_rank_matrix(extended_topics_list, rank_feature)
...
print ("Combined chapters from both Few-Chapter Book and Many-Chapter Book")
print ("Jaccard", jaccard_similarity)
print ("Cosine", cosine_simlarity)
print ("Ranked", ranked_similarity)
ranked_similarities.append(ranked_similarity)
jaccard_similarities.append(jaccard_similarity)
cosine_similarities.append(cosine_simlarity)
csv_writer.writerow(['All', len(extended_topics), ranked_similarity, jaccard_similarity, cosine_simlarity])

fig, ax1 = plt.subplots()
ax1.set_xlabel('Included Chapter')
ax1.set_ylabel('Similarity score', color = 'r')
#ax1.set_xticks(np.arange(len(xlabels)), xlabels)
ax1.set_xticks(np.arange(len(xlabels)))
ax1.set_xticklabels(xlabels)
ax1.tick_params('y', labelcolor = 'r')
ax1.plot(ranked_similarities, color ='r')
ax1.plot(jaccard_similarities, color ='g', linestyle = ':')
ax1.plot(cosine_similarities, color ='y', linestyle = '--')
ax1.legend(['Ranked', 'Jaccard', 'Cosine'], loc = 'upper left')


ax2 = ax1.twinx()
ax2.set_ylabel('Number of covered topics', color = 'b')
ax2.tick_params('y', labelcolor ='b')
ax2.plot(num_topics, color = 'b')
ax2.set_xticks(np.arange(len(xlabels)), xlabels)
ax2.legend([' Number of topics'], loc='upper right')
plt.show()
print ("Results and plots are saved in your current directory now")
fig.savefig('Similarity scores as many-chapter book with ' + argv[1] + ' chapters.png')